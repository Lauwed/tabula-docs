...
const buildFullURI = (uri, uriParameters, queryParameters, fetchContainer) => {
	let updatedURI = uri;
	...
	if (queryParameters !== undefined && queryParameters.length > 0) {
		...
		queryParameters.forEach((parameter) => {
			...
			if (
				...
				queryParts.push(`${parameter.name}=${inputValue}`);
			}
		});

		if (queryParts.length > 0) updatedURI += `?${queryParts.join('&')}`;
	}
...
const handleFetch = (
	...
	const updatedURI = buildFullURI(uri, uriParameters, queryParameters, fetchContainer);
...
const getEndpointNode = (endpoint) => {
	...
	urlContainer.classList.add('endpoint__url');
	content.insertBefore(urlContainer, fetchContainer);

	const urlTitle = document.createElement('h4');
	urlTitle.classList.add('endpoint__url__title');
	urlTitle.innerHTML = 'URL';
	urlContainer.appendChild(urlTitle);

	const urlValue = document.createElement('code');
	urlValue.classList.add('endpoint__url__value');
	...
	const updateURLDisplay = () => {
		urlValue.textContent = `${apiBaseUrl}${buildFullURI(uri, uriParameters, queryParameters, fetchContainer)}`;
	};
	updateURLDisplay();
	fetchContainer.addEventListener('input', updateURLDisplay);
	fetchContainer.addEventListener('change', updateURLDisplay);
